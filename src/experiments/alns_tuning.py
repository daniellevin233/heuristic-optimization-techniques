...
def run_alns_with_config(instance_path: str, config: ALNSConfig) -> float:
    """Run ALNS with given configuration and return objective value."""
    ...
    return best_solution.calc_objective()
...
def create_objective_function(instance_paths: list[str], time_budget: float):
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        config = ALNSConfig(
            # Weight management
            weight_update_period=trial.suggest_int('weight_update_period', 50, 500),
            reaction_factor=trial.suggest_float('reaction_factor', 0.01, 0.5),

            # Destroy parameters
            min_removal_percentage=trial.suggest_float('min_removal_pct', 0.05, 0.2),
            max_removal_percentage=trial.suggest_float('max_removal_pct', 0.3, 0.6),

            # Simulated annealing
            initial_temperature=trial.suggest_float('initial_temp', 50.0, 500.0),
            cooling_rate=trial.suggest_float('cooling_rate', 0.95, 0.9999),

            # Success scoring
            score_new_best=trial.suggest_float('score_new_best', 5.0, 20.0),
            score_accepted=trial.suggest_float('score_accepted', 0.5, 2.0),

            # Fixed parameters
            max_time_seconds=time_budget,
            max_iterations=10000,
            max_iterations_without_improvement=1000,
            log_interval=10000  # Suppress logs during tuning
        )

        # Run on all training instances and return average objective
        objectives = []
        for instance_path in instance_paths:
            obj = run_alns_with_config(instance_path, config)
...
def tune_parameters(
    ...
    """
    Tune ALNS parameters using Optuna.

    Args:
        instance_paths: List of training instance paths (relative to instances/ folder)
        time_budget: Time budget per ALNS run (seconds)
        n_trials: Number of Optuna trials
        n_jobs: Number of parallel jobs (defaults to cpu_count())
        output_dir: Directory to save results
        instance_size: Instance size for logging/saving (extracted from path if None)
    ...
    print(f"\n{'='*80}")
    print(f"Tuning ALNS parameters for {len(instance_paths)} instances (size={instance_size})")
    ...
    study_name = f"alns_tuning_n{instance_size}_trials{n_trials}_budget{int(time_budget)}s"
    ...
    # Run optimization
    # Optuna handles parallelization by running n_jobs workers that share the study
    # Each worker evaluates objective function and reports back to central TPE sampler
...
def main():
    ...
    project_root = find_project_root()
    OUTPUT_DIR = project_root / "out" / "tuning"
